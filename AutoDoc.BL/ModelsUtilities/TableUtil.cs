...
namespace AutoDoc.BL.ModelsUtilities
{
    public class TableUtil : ITableUtil
    {
        public Table GetTable(string tableContext)
        {
            ...
            //dynamic settings = JsonConvert.DeserializeObject(Structure.First) as JObject;
            //dynamic data = JsonConvert.DeserializeObject(Structure.Last) as JObject;

            //JArray tableHeading = JArray.Parse(Structure.settings.columns) as JArray;
            //JArray tableData = JArray.Parse(Structure.data.data) as JArray;
            ...
            foreach (dynamic record in tableData)
            {
                ...
                {
                    //string nameOfProperty = recordCell.Name.ToString();
                    //var propertyInfo = ((Object) record).GetType().GetProperty(nameOfProperty);
